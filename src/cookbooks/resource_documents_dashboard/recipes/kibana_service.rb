...
# Update the kibana start script to enable it to calculate its required
# memory
kibana_pid_file = "#{node['kibana']['path']['pid']}/kibana.pid"
kibana_start_script = '/usr/share/kibana/bin/elasticsearch'
file kibana_start_script do
  action :create
  content <<~TXT
    #!/bin/bash
    ...
    #
    #   ES_JAVA_OPTS="-Xms8g -Xmx8g" ./bin/kibana
...
# kibana_user = node['kibana']['service_user']
# kibana_group = node['kibana']['service_group']
service_name = node['kibana']['service_name']
...
#     environment_file '-/etc/default/kibana'
#     environment %W[
#       PID_DIR=#{node['kibana']['path']['pid']}
#     ]
